...
import { CHECKLIST_SECTIONS, CHECKLIST_PPP, CHECKLIST_PBOZP, CHECKLIST_PBOZPS, ChecklistPoint } from "./checklist-data";
...
export default function NewInspectionPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Rezim data provedeni: 'jeden' | 'rozsah' | 'vice'
    datumRezim: 'jeden' as 'jeden' | 'rozsah' | 'vice',
    datumDo: '',                         // pro rozsah
    datumVice: [] as string[],           // pro vice nenavazujicich dnu
  });
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 pb-24 relative">
      ...
      {step === 1 && (
        <Card className="border-none shadow-sm"><CardHeader><CardTitle>Základní parametry kontroly</CardTitle></CardHeader><CardContent className="space-y-6"><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="space-y-2"><Label>Klient</Label><Select value={formData.klientId} onValueChange={(v) => setFormData({...formData, klientId: v, pracovisteIds: []})}><SelectTrigger className="h-11"><SelectValue placeholder="Vyberte klienta" /></SelectTrigger><SelectContent>{klienti.map(k => <SelectItem key={k.id} value={k.id}>{k.nazev}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Pracoviště / Provozovny</Label>{!formData.klientId ? <div className="h-11 bg-muted rounded-md flex items-center px-3 text-sm italic">Nejprve zvolte klienta</div> : <div className="grid grid-cols-1 gap-2 p-3 border rounded-md bg-white max-h-[150px] overflow-y-auto">{(selectedKlient?.pracoviste || []).map((p: any) => (<label key={p.id} className="flex items-center gap-3 cursor-pointer"><Checkbox checked={formData.pracovisteIds.includes(p.id)} onCheckedChange={(c) => { setFormData(prev => ({...prev, pracovisteIds: c ? [...prev.pracovisteIds, p.id] : prev.pracovisteIds.filter(id => id !== p.id)})) }} /><span className="text-sm font-medium leading-none">{p.nazev}</span></label>))}</div>}</div><div className="space-y-2 md:col-span-2"><Label>Typ kontroly</Label><Select value={formData.typKontroly} onValueChange={(v: any) => setFormData({...formData, typKontroly: v})}><SelectTrigger className="h-11"><SelectValue placeholder="Zvolte typ kontroly" /></SelectTrigger><SelectContent><SelectItem value="BOZPaPO">BOZPaPO</SelectItem><SelectItem value="PPP">PPP</SelectItem><SelectItem value="PBOZP">PBOZP</SelectItem><SelectItem value="PBOZPS">PBOZPS (staveniště)</SelectItem><SelectItem value="KONTROLA">Vlastní (Volná kontrola)</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Datum provedení</Label>
<Select value={formData.datumRezim} onValueChange={(v: any) => setFormData({...formData, datumRezim: v})}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="jeden">Jeden den</SelectItem><SelectItem value="rozsah">Rozsah (od–do)</SelectItem><SelectItem value="vice">Více dnů (nenavazující)</SelectItem></SelectContent></Select>
{formData.datumRezim === 'jeden' && (
  <Input type="date" className="h-11" value={formData.datum} onChange={(e) => setFormData({...formData, datum: e.target.value})} />
)}
{formData.datumRezim === 'rozsah' && (
  <div className="flex gap-2 items-center">
    <Input type="date" className="h-11" value={formData.datum} onChange={(e) => setFormData({...formData, datum: e.target.value})} />
    <span className="text-slate-400">–</span>
    <Input type="date" className="h-11" value={formData.datumDo} onChange={(e) => setFormData({...formData, datumDo: e.target.value})} />
  </div>
)}
{formData.datumRezim === 'vice' && (
  <div className="space-y-2">
    {(formData.datumVice.length > 0 ? formData.datumVice : ['']).map((d, i) => (
      <div key={i} className="flex gap-2 items-center">
        <Input type="date" className="h-11" value={d} onChange={(e) => { const next = [...(formData.datumVice.length > 0 ? formData.datumVice : [''])]; next[i] = e.target.value; setFormData({...formData, datumVice: next}); }} />
        {(formData.datumVice.length > 1) && <Button variant="ghost" size="icon" onClick={() => setFormData({...formData, datumVice: formData.datumVice.filter((_, idx) => idx !== i)})} className="shrink-0 text-muted-foreground hover:text-red-500"><X className="h-4 w-4" /></Button>}
      </div>
    ))}
